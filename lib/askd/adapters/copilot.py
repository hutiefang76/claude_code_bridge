...
Wraps existing haskd_* modules to provide a consistent interface.
"""
...
from haskd_protocol import extract_reply_for_req, is_done_text, wrap_copilot_prompt
from haskd_session import compute_session_key, load_project_session
from copilot_comm import CopilotLogReader
...
class CopilotAdapter(BaseProviderAdapter):
    ...
    def compute_session_key(self, session: Any, instance: Optional[str] = None) -> str:
        return compute_session_key(session, instance) if session else "copilot:unknown"

    def handle_task(self, task: QueuedTask) -> ProviderResult:
        started_ms = _now_ms()
        req = task.request
        work_dir = Path(req.work_dir)
        _write_log(f"[INFO] start provider=copilot req_id={task.req_id} work_dir={req.work_dir}")
        ...
        rebounded = False
        tail_bytes = int(os.environ.get("CCB_HASKD_REBIND_TAIL_BYTES", str(2 * 1024 * 1024)))
        pane_check_interval = float(os.environ.get("CCB_HASKD_PANE_CHECK_INTERVAL", "2.0"))
        ...
        _write_log(f"[INFO] done provider=copilot req_id={task.req_id} exit={result.exit_code}")
        return result
